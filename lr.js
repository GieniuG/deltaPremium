...
function clearCards(){
    ...
    document.querySelectorAll(".possible").forEach(n => n.classList.remove("possible"))
}
function cardHelper() {
        ...
        document.querySelectorAll("#cards .card").forEach((card, index) => {
            card.addEventListener("click", () => {
                ...
                for (let i = 0; i < cardsLeft.length / 2; i++) {
                        let idx = 2 * Math.floor(Math.random() * Math.floor(cardsLeft.length / 2)) + !(index % 2)
    ...
    observer.observe(document.querySelector("#cards"),{subtree:true,attributes:true,attributeFilter:["class"]})
}
